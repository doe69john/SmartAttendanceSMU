...
public class AttendanceProperties {
    ...
    private static Path detectConfigFile() {
        ...
        Path companionRelative = Paths.get("companion", "runtime", "config.properties");
        if (Files.exists(companionRelative)) {
            return companionRelative.toAbsolutePath().normalize();
        }
    ...
    private ConfigSnapshot loadSnapshot() {
        ...
        double autoAcceptMax = getDouble(props, "recognition.auto.max_distance", 500.0);
        double manualReviewMax = getDouble(props, "recognition.manual.max_distance", 520.0);
        if (manualReviewMax < autoAcceptMax) {
            manualReviewMax = autoAcceptMax;
        }

        Recognition recognition = new Recognition(
                Math.max(0.0, autoAcceptMax),
                Math.max(0.0, manualReviewMax),
                new Lbph(
    ...
    public record Recognition(double autoAcceptMaxDistance,
                              double manualReviewMaxDistance,
                              Lbph lbph) {
        public double autoAcceptMaxDistance() {
            return Math.max(0.0, autoAcceptMaxDistance);
        }
        public double manualReviewMaxDistance() {
            return Math.max(autoAcceptMaxDistance(), manualReviewMaxDistance);
        }
    }
