...
const Reports = () => {
  ...
  const { data: studentReports = [], isLoading: loadingStudents } = useQuery<ProfessorStudentReport[]>({
    queryKey: ['reports', 'students', shouldQueryStudents ? deferredStudentQuery : ''],
    enabled: shouldQueryStudents,
    queryFn: async () => {
      const result = await fetchProfessorStudentReports({
  ...
  const { data: sectionReports = [], isLoading: loadingSections } = useQuery<ProfessorSectionReport[]>({
    queryKey: ['reports', 'sections', shouldQuerySections ? deferredSectionQuery : ''],
    enabled: shouldQuerySections,
    queryFn: async () => {
      const result = await fetchProfessorSectionReports({
  ...
  const studentDetailQuery = useQuery<StudentReportDetail | null>({
    ...
    queryFn: async () => {
      ...
      return fetchStudentReportDetail(selectedStudentId);
    },
    staleTime: 120_000,
  });
  ...
  const sectionDetailQuery = useQuery<SectionReportDetail | null>({
    ...
    queryFn: async () => {
      ...
      return fetchSectionReportDetail(selectedSectionId);
    },
    staleTime: 120_000,
  });
  ...
  const handleDownload = useCallback(
    async (params: { kind: 'student' | 'section'; id: string; format: 'csv' | 'xlsx' }) => {
      try {
        ...
        const result =
          params.kind === 'student'
            ? await downloadStudentReport(params.id, params.format)
            : await downloadSectionReport(params.id, params.format);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          Comprehensive attendance analytics for {profile?.fullName ?? 'your account'}.
        </p>
      </div>
