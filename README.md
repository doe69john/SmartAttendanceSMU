...
| Batch Roster Import           | Upload CSV/XLSX rosters, deduplicate rows, resolve students by email/ID/name, and return actionable matches/issues | `TeachingController` (`/api/sections/roster-import`), `RosterImportService`, `RosterImportResponse`, `StudentDirectoryService` | Bonus     |
...
| Session Policy/rules Enforcement    | Enforce ownership checks, 30‑minute start windows, roster seeding, and late thresholds,session de-duplication   | `SessionLifecycleService`, `SessionLifecycleController`, `AttendanceService`, `AttendanceRecordEntity`                         | Bonus      |
